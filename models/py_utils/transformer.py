...
class Transformer(nn.Module):
    ...
    def save_params(self, file):
        self.encoder.save_params(file)
        self.decoder.save_params(file)
...
class TransformerEncoder(nn.Module):
    ...
    def save_params(self,file):
        for i in range(0,self.num_layers):
            self.layers[i].save_params(file)
        if self.norm != None :
            save_norm_layer(self.norm, file)
...
class TransformerEncoderLayer(nn.Module):
    ...
    def save_params(self,file):
        save_multiheadatten(self.self_attn, file)
        save_linear(self.linear1,file)
        save_linear(self.linear2,file)
        save_norm_layer(self.norm1, file)
        save_norm_layer(self.norm2, file)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def save_params(self,file):
        save_multiheadatten(self.self_attn, file)
        save_multiheadatten(self.multihead_attn, file)
        save_linear(self.linear1,file)
        save_linear(self.linear2,file)
        save_norm_layer(self.norm1, file)
        save_norm_layer(self.norm2, file)
        save_norm_layer(self.norm3, file)
    def with_pos_embed(self, tensor, pos: Optional[Tensor]):
        return tensor if pos is None else tensor + pos
