...
def xy_wh_r_2_xy_sigma(xywhr):
    ...
    S = 0.5 * torch.diag_embed(wh) #n,2,2
...
def kld_loss(pred, target, alpha=1.0, sqrt=True):
    ...
    log(|sigam2|/|sigam1|)-n+tr(sigam2_inverse*sigam1)+(u2-u1).t*sigam2_inverse*(u2-u1)
    Returns:
        loss (torch.Tensor)
    """
    xy_p, Sigma_p = pred
    ...
    Sigma_p_inv = torch.stack((Sigma_p[..., 1, 1], -Sigma_p[..., 0, 1],
                              ...
                              dim=-1).reshape(-1, 2, 2)
    Sigma_p_inv = Sigma_p_inv / (1e-25 + torch.linalg.det(Sigma_p)).unsqueeze(-1).unsqueeze(-1)
    ...
    Sigma_p_det_log = torch.linalg.det(Sigma_p).log()
    Sigma_t_det_log = torch.linalg.det(Sigma_t).log()
