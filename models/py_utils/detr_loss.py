...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_curves, log=True):
        ...
        src_logits0 = src_logits[..., :self.num_classes]
        idx = self._get_src_permutation_idx(indices)
        # target_classes_o = torch.cat([tgt[:, 0][J].long() for tgt, (_, J) in zip (targets, indices)])
        target_classes_o = torch.cat([tgt[:, 0][J].long()//10 for tgt, (_, J) in zip (targets, indices)])
        target_classes = torch.full(src_logits0.shape[:2], 0, dtype=torch.int64, device=src_logits0.device)
        target_classes[idx] = target_classes_o
        # print(src_logits0.shape, target_classes.shape)
        loss_ce0 = F.cross_entropy(src_logits0.transpose(1, 2), target_classes, self.empty_weight)
        # print(loss_ce0)

        src_logits1 = src_logits[..., self.num_classes:-1] #增加-1
        # idx = self._get_src_permutation_idx(indices)
        target_classes_o = torch.cat([tgt[:, 0][J].long()%10 for tgt, (_, J) in zip (targets, indices)])
        target_classes = torch.full(src_logits1.shape[:2], 0, dtype=torch.int64, device=src_logits1.device)
        target_classes[idx] = target_classes_o
        loss_ce1 = F.cross_entropy(src_logits1.transpose(1, 2), target_classes, self.empty_weight)
        # loss = loss_ce0 + loss_ce1
        losses = {'loss_ce0': loss_ce0, 'loss_ce1': loss_ce1}
    ...
    def loss_curves(self, outputs, targets, indices, num_curves):
        ...
        src_polys  = outputs['pred_curves'][:, :, 2:-1][idx]
        src_breakpoints  = outputs['pred_curves'][:, :, -1][idx]
        src_exist_b = outputs['pred_logits'][..., -1][idx].sigmoid()
        ...
        target_points = torch.cat([tgt[:, 3:-1][i] for tgt, (_, i) in zip(targets, indices)], dim=0)
        target_breakpoints = torch.cat([tgt[:, -1][i] for tgt, (_, i) in zip(targets, indices)], dim=0)
        ...
        # print(src_polys.shape)
        pred_xs = src_polys[:, 0] / (ys - src_polys[:, 1]) ** 2 + src_polys[:, 2] / (ys - src_polys[:, 1]) + \
                  src_polys[:, 3] + src_polys[:, 4] * ys - src_polys[:, 1]*src_polys[:, 4]
        ...
        diff1 = -2 * src_polys[:, 0] / (ys - src_polys[:, 1]) ** 3 \
                - src_polys[:, 2] / (ys - src_polys[:, 1]) ** 2 \
                + src_polys[:, 4]
        diff2 = 6 * src_polys[:, 0] / (ys - src_polys[:, 1]) ** 4 \
                + 2 * src_polys[:, 2] / (ys - src_polys[:, 1]) ** 3
        ...
        loss_exist_b = self.floss(src_exist_b, breakpoint_t.long())
        losses = {}

        #loss_not_breakpoints = F.l1_loss(src_breakpoints[~breakpoint_t], target_breakpoints[~breakpoint_t], reduction='none')
        # loss_polys  = F.l1_loss(pred_xs[valid_xs], target_xs[valid_xs], reduction='none')
        loss_polys = F.l1_loss(pred_xs[valid_xs] * (detached_pred_diffs[valid_xs] + 1),
                            target_xs[valid_xs] * (detached_pred_diffs[valid_xs] + 1), reduction='none')

        losses['loss_lowers']  = loss_lowers.sum() / num_curves
        losses['loss_uppers']  = loss_uppers.sum() / num_curves
        if breakpoint_t.sum():
            loss_breakpoints = F.l1_loss(src_breakpoints[breakpoint_t], target_breakpoints[breakpoint_t], reduction='sum')
        else:
            loss_breakpoints = loss_exist_b.new_zeros(losses['loss_uppers'].shape)
            breakpoint_t = ~breakpoint_t

        #loss_breakpoints = loss_exist_b.new_zeros(losses['loss_uppers'].shape, requires_grad=True) + 1
        losses['loss_breakpoints']  = (loss_breakpoints * 10 / breakpoint_t.sum())
        losses['loss_curves']   = loss_polys.sum() / num_curves
        losses['loss_exist_b'] = loss_exist_b.sum()
