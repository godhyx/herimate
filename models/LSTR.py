...
class model(kp):
    def __init__(self, flag=False):
        ...
        layers          = system_configs.res_layers#[2, 2, 2, 2]
        res_dims        = system_configs.res_dims#[64, 128, 256, 512]
        res_strides     = system_configs.res_strides#[1, 2, 2, 2]
        attn_dim        = system_configs.attn_dim#64
        dim_feedforward = system_configs.dim_feedforward#4 * 64

        num_queries = system_configs.num_queries  # number of joints100
        drop_out    = system_configs.drop_out#0.1
        num_heads   = system_configs.num_heads#8
        enc_layers  = system_configs.enc_layers#6
        dec_layers  = system_configs.dec_layers#6
        lsp_dim     = system_configs.lsp_dim#8
        mlp_layers  = system_configs.mlp_layers#3
        lane_cls     = system_configs.lane_categories#10
